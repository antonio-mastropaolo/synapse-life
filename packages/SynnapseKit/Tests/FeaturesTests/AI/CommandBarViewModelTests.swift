...
struct CommandBarViewModelTests {
    ...
    private func makeVM() -> CommandBarViewModel {
        CommandBarViewModel(
            askAPI: LocalStubAskAPI(),
    ...
    @Test func submitStartsStreamingAndProducesAnswer() async throws {
        ...
        vm.submit()
        // Poll up to 3s for the stream to land tokens.
        let deadline = Date().addingTimeInterval(3.0)
        while vm.streamingAnswer.isEmpty && Date() < deadline {
