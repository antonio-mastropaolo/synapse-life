...
Worktree: `/Users/amastro/Projects/Synapse-worktrees/ios-mobile-redesign`
...
or live entirely in `apps/Synapse-iOS/Features/`.
...
- `apps/Synapse-iOS/Features/FinanceHubView.swift` — 4-card Finance hub
...
- `apps/Synapse-iOS/Features/AccountDetailView.swift` — per-account
...
- `apps/Synapse-iOS/Features/TransactionDetailView.swift` — per-row
  drill-down with hero amount and field stack.
- `apps/Synapse-iOS/Features/PositionDetailView.swift` — per-position
...
- `apps/Synapse-iOS/SynapseiOSApp.swift` — 4 tabs (Finance / Life /
